...
function checkOverallSize(){
	...
	if(totalSize > maxTotalSize){
		alert('Vous avez dépassé la taille de fichier maximale permise pour un partage./You have exceeded the maximum filesize you can send in one dropoff (' + formatFileSize(totalSize) + ').');
		return false;
	} else {
...
function insertNewFileRow(){
	...
	f.append('<input type="file" name="file_' + file_id + '" id="file_' + file_id + '" size="50" onChange="addFile(\'file_' + file_id + '\');"/> ');
 	if(usingLibrary == "1") f.append('<select id="file_select_' + file_id + '" name="file_select_' + file_id + '" class="file_select"><option value="-1">ou sélectionnez un fichier</option></select> ');
...
<div class="fright">
	<input type="checkbox" name="informRecipients" id="informRecipients" checked="checked" /> <label for="informRecipients">Envoyer un courriel au destinataire(s)</label><br />
	<input type="checkbox" name="confirmDelivery" id="confirmDelivery" checked="checked"/> <label for="confirmDelivery">M'envoyer un courriel lorsque le destinataire télécharge le fichier partagé</label>
</div>
...
<div id="addNewRecipient">
	<h1>Ajouter destinataires</h1>
	<p id="buttonHolder" class="center">
		<button id="showSingleDialog">Ajouter</button> <button id="showMultipleDialog" class="greyButton">Ajouter plusieurs</button>
	</p>
	
	<!-- Sending to a single recipient -->
	<div id="sendSingle" class="center">
	<div>
		<label for="recipName" class="UILabel">Nom:</label>
    	<input type="text" id="recipName" name="recipName" size="30" value="{$recipName_1}"/>
    </div>
    <div> 
    	<label for="recipEmail" class="UILabel">Courriel:</label>
    	<input type="text" id="recipEmail" name="recipEmail" size="30" value="{$recipEmail_1}"/> 
    </div>
    <button onclick="javascript:addSingleRecipient();">Ajouter destinataire</button>
	</div>
	
	<div id="sendMultiple" class="center">
	<textarea id="multipleRecipients" rows="10" cols="40" placeholder="Bulk add recipients"></textarea>
	<p>Un destinataire par ligne, au format: <br /><i>Destinataire Test destinataire.test@domaine.com</i></p>
		<div class="center"><button id="addMultipleRecipients">Valider</button></div>	
	</div>
</div>

<p>
	<label for="note">Note pour destinataires</label>
	<textarea name="note" id="note" wrap="soft" style="width:99%;height: 50px">{$note}</textarea>
</p>

<b>Choisissez les fichiers à partager <br/>Choose the File(s) you would like to upload</b>
...
<div id="uploadFiles">
	<div id="div_file_1" class="file">
		...
		<select id="file_select_1" name="file_select_1" class="file_select">
			<option value="-1">ou sélectionnez un fichier</option>
		</select><br/>
...
<div class="center"><button onclick="submitform();">Partager</button></div>
