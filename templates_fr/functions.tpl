...
{function name=footerButtons}
  ...
  <td>{call name=button href="{$zendToURL}" text="Retour au menu principal de {#ServiceTitle#}"}</td>
  {if $isAuthorizedUser}<td>{call name=button href="{$zendToURL}?action=logout" text="Déconnexion"}</td>{/if}</tr>
  </table>
{/function}
